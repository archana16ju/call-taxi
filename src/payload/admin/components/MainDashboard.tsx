...
export default function MainDashboard() {
  ...
  React.useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const drivers = await fetch('/api/drivers?limit=5').then((res) => res.json())

        setDriversList(drivers.docs || [])
      } catch (err) {
        console.error(err)
      } finally {
        setLoading(false)
  ...
  const fetchLocations = React.useCallback(async () => {
    try {
      ...
      await Promise.all(
        driversRes.docs.map(async (driver: any) => {
          try {
            // must have valid live gps
            if (!driver.location || !Array.isArray(driver.location) || driver.location.length < 2) {
              return
            }

            // match booking with driver
            const booking = bookingsRes.docs.find((b: any) => {
              const driverId = typeof b.driver === 'object' ? b.driver?.id : b.driver
            ...
            if (
              ...
              booking.dropoffLocation.length < 2
            ) {
              return
            }
            ...
            if (routeData.routes && routeData.routes[0] && routeData.routes[0].geometry) {
              routes[driver.id] = routeData.routes[0].geometry.coordinates.map(
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            {/* Administrator */}
            <Link href="/admin/collections/users" style={{ textDecoration: 'none' }}>
              <Stack
                direction="row"
                spacing={1.2}
                alignItems="center"
      ...
      <Grid container spacing={3} sx={{ px: 3, pb: 3 }}>
        ...
        <Grid size={{ xs: 12, sm: 6, lg: 2.4 }}>
          <StatCard
            title="Ongoing"
            value={stats.ongoingBookings.toLocaleString()}
            trend="down"
            trendValue="-6.2%"
            icon={<AccessTimeIcon />}
            color="#f59e0b"
              ...
              {driversList.length > 0 ? (
                driversList.map((driver, i) => (
                  <Stack
                    ...
                    justifyContent="space-between"
                    alignItems="center"
                    sx={{ p: 1, borderRadius: '10px', '&:hover': { backgroundColor: '#f8fafc' } }}
                  >
                    <Stack direction="row" spacing={1.5} alignItems="center">
                      ...
                      <Box>
                        <Typography
                          variant="caption"
                          sx={{ fontWeight: 800, color: '#1e293b', display: 'block' }}
                        >
                          {driver.name}
                        </Typography>
                        <Typography
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                      <Typography
                        ...
                        sx={{
                          fontWeight: 800,
                          color: driver.status === 'available' ? '#10b981' : '#3b82f6',
                          fontSize: '0.65rem',
                          display: 'flex',
                          alignItems: 'center',
                          gap: 0.5,
                ...
                <Typography variant="caption" sx={{ color: '#64748b', textAlign: 'center', py: 4 }}>
                  No data found
                </Typography>
              )}
            </Stack>
            ...
            <Grid container spacing={1}>
              <Grid size={3}>
                ...
                >
                  <Typography
                    variant="caption"
                    sx={{ color: '#64748b', display: 'block', fontSize: '0.6rem' }}
                  >
                    Total Drivers
                  </Typography>
              ...
              <Grid size={3}>
                ...
                >
                  <Typography
                    variant="caption"
                    sx={{ color: '#10b981', display: 'block', fontSize: '0.6rem' }}
                  >
                    Online
                  </Typography>
              ...
              <Grid size={3}>
                ...
                >
                  <Typography
                    variant="caption"
                    sx={{ color: '#ef4444', display: 'block', fontSize: '0.6rem' }}
                  >
                    Offline
                  </Typography>
              ...
              <Grid size={3}>
                ...
                >
                  <Typography
                    variant="caption"
                    sx={{ color: '#8b5cf6', display: 'block', fontSize: '0.6rem' }}
                  >
                    Vehicles
                  </Typography>
                  <Typography variant="body2" sx={{ fontWeight: 900, color: '#8b5cf6' }}>
                    85
                  </Typography>
                </Box>
              </Grid>
          ...
          >
            <Box sx={{ p: 2, position: 'absolute', top: 0, left: 0, right: 0, zIndex: 1 }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <Button
                  component={Link}
                  href="/app/frontend/live-tracking"
                  size="small"
                  sx={{ textTransform: 'none', color: '#3b82f6', fontWeight: 700 }}
                >
                  View Full Map
    ...
    `,
            ...
            <Stack
              ...
              spacing={2}
              justifyContent="center"
              sx={{ py: 1.5, backgroundColor: '#f8fafc', borderTop: '1px solid #e2e8f0' }}
            >
              <Typography
                variant="caption"
              ...
              >
                <Box
                  sx={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: '#3b82f6' }}
                />{' '}
                On Trip (56)
              </Typography>
              <Typography
              ...
              >
                <Box
                  sx={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: '#10b981' }}
                />{' '}
                Available (28)
              </Typography>
              <Typography
              ...
              >
                <Box
                  sx={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: '#94a3b8' }}
                />{' '}
                Offline (32)
              </Typography>
            </Stack>
          ...
          >
            ...
            <Stack spacing={2}>
              {[
                {
                  title: 'SOS Alert',
                  msg: 'Driver Suresh Babu triggered SOS',
                  type: 'sos',
                  color: '#ef4444',
                  icon: <NotificationsNoneIcon />,
                },
                {
                  title: 'High Cancellation',
                  msg: 'Driver Karthik has high cancellation',
                  type: 'warn',
                  color: '#f59e0b',
                  icon: <NotificationsNoneIcon />,
                },
                ...
                {
                  title: 'New Booking',
                  msg: 'New booking #BK-250526-1248',
                  type: 'book',
                  color: '#10b981',
                  icon: <NotificationsNoneIcon />,
                },
                ...
                <Stack key={i} direction="row" spacing={1.5} alignItems="flex-start">
                  ...
                  <Box>
                    <Typography
                      variant="caption"
                      sx={{ fontWeight: 900, color: '#1e293b', display: 'block' }}
                    >
                      {alert.title}
                    </Typography>
                    <Typography
                      variant="caption"
                      sx={{ color: '#64748b', fontSize: '0.65rem', display: 'block' }}
                    >
                      {alert.msg}
                    </Typography>
                  </Box>
            ...
            <Grid size={{ xs: 12, md: 12 }}>
              ...
              >
                ...
                <Typography variant="caption" sx={{ color: 'var(--theme-text-secondary)' }}>
                  All data is pulled directly from your active collections.
                </Typography>
              </Paper>
            </Grid>
          </Grid>
